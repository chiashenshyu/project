...
void ParticleFilter::observation(const States& n, 
                                 const std::vector<vector<double>>& landmark,
                                 const std::vector<bool>& lmVisible){
    z.resize(landmark.size()); 
    std::default_random_engine generator(time(0));
    std::uniform_real_distribution<double> distribution(0.0, 1);
    for(int i = 0; i < landmark.size(); i++){
        if(!lmVisible[i]){
            std::vector<double> tmp = {-1, landmark[i][0], landmark[i][1]};
            z[i] = tmp; 
            continue;
        }
